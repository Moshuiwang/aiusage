...
## 免费版资源预算与日常巡检不变量（硬性规则）

本项目线上后端承载于 **Cloudflare Free Plan（免费版）**，所有优化、客户端修改和功能扩展必须严格遵守资源预算，严防超标导致降级或封禁：

- **免费版核心预算指标**：
  - Workers 请求量：100,000 次/天（当前 ~2.2% 水位）
  - Workers CPU 耗时：10 ms / 次（当前 P50 ~1.23 ms）
  - D1 每日写入：100,000 行/天（当前 ~29.4% 水位，**最需关注的指标**）
  - D1 每日读取：5,000,000 行/天（当前 ~22.2% 水位）
  - D1 存储空间：500 MB / 库（当前 ~1.8% 水位）
  - R2 备份存储：10 GB / 月（当前 ~0.1% 水位）

- **开发与优化原则**：
  1. **展示端 / 客户端零滥用**：客户端（Mac Popover / iPhone / Web）只读展示，严禁短轮询、严禁重复高频触发全量读。
  2. **上报端批量聚合**：各设备数据上报必须批量提交，避免频繁单行写 D1。
  3. **上线必巡检（SOP）**：任何涉及接口改动、上报协议或客户端更新上线后，必须在 Mac 侧执行巡检脚本核实水位（交付证据等级 7 回源确认）：
     ```bash
     python3 scripts/check_cf_usage.py
     ```
