...
项目已完成个人 HTTP push 架构的基础部署，并完成 official limits provider MVP 的离线可测基线、runtime wiring、Codex WHAM 显式 auth adapter、Claude OAuth 显式 auth adapter、Codex app-server RPC adapter、Claude CLI `/usage` adapter、limits 本地配置契约、limits dry-run、scheduler 模板和真实 smoke handoff。
...
- 工程顺序：HTTP ingest、终端 pusher、canonical store / snapshot、Web dashboard 已形成 baseline；official limits contract、Codex / Claude offline provider parser、limits store、snapshot/API、Web 展示、`collect-limits` fixture runtime、Codex WHAM 显式 auth adapter、Claude OAuth 显式 auth adapter、Codex app-server RPC adapter、Claude CLI `/usage` adapter、limits 本地配置契约、dry-run、scheduler 模板和真实 smoke handoff 已形成 baseline。
