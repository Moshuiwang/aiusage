...
describe.sequential("today 小时趋势（活路径）", () => {
  it("账本小时事实原样落到对应的那个小时点上，四个数一个都不许变形", async () => {
    await withWorker({ now: fixedNow }, async ({ db }) => {
      ...
      // **诚实标注**：这一条我没能找到让它变红的变异。试过两种——「趋势里丢掉一条事实」
      // 与「周期总量虚增制造残差」——残差都被摊到**已经有数据的小时**上，
      // 09:00 / 14:00 那两条先失败，这一条从未被求值。
      // 它不是恒真（03:00 理论上可以非零：残差若改成按 24 小时均摊就会），
      // 但它目前是一条**未经变异验证**的断言，别把它算进「已守住」。
