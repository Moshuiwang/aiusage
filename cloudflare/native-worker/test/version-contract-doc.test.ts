...
describe("版本合同文档 · 服务端半边（#90 块 2）", () => {
  it("文档存在、被架构总文档引用，且本测试依赖的章节一个不少", () => {
    ...
    // 结构下限：章节标题被改名时，下面每条用例都会因为「缺少章节」抛错而不是静默变松，
    // 这里把这件事变成一条显式断言，免得整组用例的失败原因都指向同一个无关的解析错误。
    const text = readDoc();
    const required = [
    ...
    const present = required.filter((heading) => section(text, heading).trim().length > 0);
    expect(present).toEqual(required);
  });
